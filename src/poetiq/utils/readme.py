...
class Readme:
    ...
    def add_poetiq_line(self, prep: str):
        """
        Add a made with poetiq line to README if does not exist.
        """
        poetiq_line = f"*Made {prep} {POETIQ_LINK}*\n"

        if not self._file.has_line(poetiq_line):
            lines = []
            lines.append("\n-----\n")
            lines.append(poetiq_line)
